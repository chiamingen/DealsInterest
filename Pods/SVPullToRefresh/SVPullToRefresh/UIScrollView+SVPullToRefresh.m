...
- (void)layoutSubviews {
    ...
    arrowFrame.origin.x = ceilf(remainingWidth*position+25);
    self.arrow.frame = arrowFrame;

    //self.activityIndicatorView.center = self.arrow.center;
    //NSLog(@"self.superview.bounds.size.width = %f", self.superview.bounds.size.width);
    
    CGRect actFrame = self.activityIndicatorView.frame;
    actFrame.origin.x = ceilf(screenWidth/2 - (actFrame.size.width/2));
    actFrame.origin.y = self.bounds.size.height-(self.subtitleLabel.text ? 48 : 40);
    self.activityIndicatorView.frame = actFrame;
